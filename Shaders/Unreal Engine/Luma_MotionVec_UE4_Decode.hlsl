...
Texture2D<float4> VelocityTexture : register(VELOCITY_TEX_INDEX); // Velocity texture
Texture2D<float4> DepthTexture : register(DEPTH_TEX_INDEX); // Depth buffer


SamplerState s0_s : register(s0);

cbuffer cb1 : register(GLOBAL_CB_INDEX)
{
  float4 cb1[GLOBAL_CB_SIZE];
}
...
float2 main(float4 pos : SV_Position0) : SV_Target0
{
    ...
    float2 TemporalJitterPixels = cb1[JITTER_CB_INDEX].zw * LumaData.GameData.RenderResolution.xy * float2(0.5, -0.5);
    ...
    float4x4 ClipToPrevClip = float4x4(cb1[CLIP_TO_PREV_CLIP_CB_START_INDEX], cb1[CLIP_TO_PREV_CLIP_CB_START_INDEX + 1], cb1[CLIP_TO_PREV_CLIP_CB_START_INDEX + 2], cb1[CLIP_TO_PREV_CLIP_CB_START_INDEX + 3]);
